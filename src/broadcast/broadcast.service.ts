...
export class BroadcastService {
    ...
    constructor(private readonly networkService: NetworkService, private eventEmitter: EventEmitter2) {}
    ...
    async testBroadcast() {
        ...
        this.logger.verbose(
            `acknowledged: ${acknowledged}, broadcasted: ${broadcasted}, failed: ${failed}, total: ${total}`,
        );
    }
    ...
    async broadcastEvent(event: string, payload: any) {
        ...
        for (const peer of peers) {
            const request = this.networkService
                .request('POST', `/broadcast`, encapsulatedData, peer)
                .then((success) => {
                    if (success) {
                        broadcasted++;
                    } else {
