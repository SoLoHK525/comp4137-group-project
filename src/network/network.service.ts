...
export class NetworkService {
    ...
    handshake(target: string) {
        ...
        return this.request<boolean>("POST", "/network/handshake", {
    ...
    sendHandshakeVerification(target: string, code: string) {
        ...
        return this.request<boolean>("POST", "/network/handshakeVerify", {
            address: this.address,
            code
        }, target).then(success => {
            if (success) {
            ...
            this.logger.error(`Failed to send handshake verification: ${err.response?.data.message || err.message}`);
        })
    }
    ...
    sendChallenge(address: string) {
        ...
        this.request("POST", "/network/handshakeChallenge", {
            address: this.address,
            code: challenge.code
        }, address).catch((err) => {
    ...
    requestPeerList(source: string) {
        this.request<string[]>("GET", "/network/peers", null, source).then(peers => {
            peers.forEach(peer => {
                this.peers.add(peer)
            });
        }).catch(err => {
